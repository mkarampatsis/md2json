...
Check if a given string is a valid hexadecimal color code. Print 'Valid hexadecimal color code.' if true else 'Invalid hexadecimal color code.' if false. (Input: '#FF0000')
...
### Hint 2
Use the `match()` function to check if the pattern matches the entire color code.
