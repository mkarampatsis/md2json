...
Extract all words starting with 'cat' from a given string. (Input: 'The cat sat on the mat. The caterpillar is cute.')

Expected output:
```
Words starting with 'cat': [words that start with cat]
```
...
input_string = 'The cat sat on the mat. The caterpillar is cute.'
pattern = re.compile(r'\bcat\w*')
words = pattern.findall(input_string)
print('Words starting with \'cat\':', words)
```

## Output Data
type=oneline
Words starting with 'cat': ['cat', 'caterpillar']
...
### Hint 2
Use the `findall()` function to find all words starting with 'cat' in the string.
