...
Count the number of digits in a given string. (Input: 'Hello123World456')

Expected output:
```
Number of digits: [number of digits found]
```
...
digits = pattern.findall(input_string)
...
### Hint 2
Use the `findall()` function to find all digits in the string.
