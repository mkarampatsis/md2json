...
Check if a given string is a valid username (alphanumeric characters only, no spaces). If valid print 'Valid username.' else 'Invalid username.' (Input: 'john_doe123')

## Code
```py3
import re

input_string = 'john_doe123'
...
### Hint 2
Use the `match()` function to check if the pattern matches the entire username.
