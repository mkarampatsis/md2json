...
Check if a given string is a valid credit card number (Visa or MasterCard). Print 'Valid credit card number.' if valid else 'Invalid credit card number.'. (Input: '4111 1111 1111 1111')
...
pattern = re.compile(r'^4\d{3}(?:[\s-]?\d{4}){3}$|^5[1-5]\d{2}(?:[\s-]?\d{4}){3}$')
...
### Hint 2
Use the `match()` function to check if the pattern matches the entire credit card number.
