...
Validate if a given string represents a valid IPv4 address. (Input: '192.168.0.1')

Print 'Valid IPv4 address.' if valid else 'Invalid IPv4 address.'.
...
pattern = re.compile(r'^((\d{1,2}|1\d{2}|2[0-4]\d|25[0-5])\.){3}(\d{1,2}|1\d{2}|2[0-4]\d|25[0-5])$')
...
### Hint 2
Use the `match()` function to check if the pattern matches the entire IPv4 address.
