...
Extract all words that start with a vowel from a given string. (Input: 'Hello world! Apple and orange are fruits.')

Expected output:
```
Words starting with a vowel: [words starting with a vowel]
```
...
input_string = 'Hello world! Apple and orange are fruits.'

pattern = re.compile(r'\b[aeiouAEIOU]\w+\b')
vowel_words = pattern.findall(input_string)
print('Words starting with a vowel:', vowel_words)
```

## Output Data
type=oneline
Words starting with a vowel: ['Apple', 'and', 'orange', 'are']
...
### Hint 2
Use the `findall()` function to find all words starting with a vowel in the string.
