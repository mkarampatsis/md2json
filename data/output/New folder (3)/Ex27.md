...
Validate if a given string represents a valid date in the format 'YYYY-MM-DD'. Print 'Valid date.' if valid else 'Invalid date.'. (Input: '2023-05-23')

## Code
```py3
import re

input_string = '2023-05-23'
...
### Hint 2
Use the `match()` function to check if the pattern matches the entire date.
