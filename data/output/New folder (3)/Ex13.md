...
Validate if a given string represents a valid URL. (Input: 'https://www.example.com')

Print 'Valid URL.' if valid else 'Invalid URL.'.
## Code
```py3
import re

input_string = 'https://www.example.com'
...
### Hint 2
Use the `match()` function to check if the pattern matches the entire URL.
